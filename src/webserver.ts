...
router.post("/api/v1/execute", async (context: Context) => {
  ...
  const highBytePush = fetch(`${WRITE_MEMORY_API}?address=${value["state"]["stackPointer"]}&value=${value["state"]["programCounter"] >> 8}`);

  // Push PC low byte
  value["state"]["stackPointer"] = value["state"]["stackPointer"] -= 1;
  if (value["state"]["stackPointer"] < 0) value["state"]["stackPointer"] += 0xFFFF;
  const lowBytePush = fetch(`${WRITE_MEMORY_API}?address=${value["state"]["stackPointer"]}&value=${value["state"]["programCounter"] & 0xFF}`);
